#!/usr/bin/env python3
"""
Footer Shell widget for Agent TUI.

Provides a persistent footer bar with command input that expands to a semi-modal
overlay when the user starts typing commands.
"""
...
class FooterShell(Container):
    """
    Persistent footer shell that expands to semi-modal overlay.

    Features:
    - Compact footer bar with command input
    - Expands to semi-modal overlay when focused/typing
    - Command history navigation (up/down arrows)
    - Tab completion for commands (agents, spawn, pause, resume, cancel, clear)
    - Collapses back to footer on completion or Escape
    ...
    FooterShell.collapsed {
        height: 3;
    }

    FooterShell.expanded {
        height: 60%;
        border: tall #4a4a6a;
    }

    FooterShell #shell-output-container {
        display: none;
        height: 1fr;
        width: 1fr;
    }

    FooterShell.expanded #shell-output-container {
        display: block;
    }

    FooterShell #shell-output {
        height: 1fr;
        width: 1fr;
        background: #1a1a2e;
        padding: 0 1;
        ...
        scrollbar-color-active: #8a8aaa;
    }

    FooterShell #output-text {
        width: 1fr;
        background: transparent;
        color: #e0e0e0;
    }

    FooterShell #completions-popup {
        display: none;
        width: 1fr;
        height: auto;
        background: #2a2a4e;
    ...
    FooterShell #completions-popup.visible {
        display: block;
    }

    FooterShell #prompt-container {
        height: 3;
        width: 1fr;
        background: #1a1a2e;
        padding: 1 1;
        border-top: solid #4a4a6a;
    }

    FooterShell #shell-prompt {
        width: auto;
        color: #00ff00;
        text-style: bold;
        background: transparent;
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize the footer shell widget.
    ...
    def compose(self):
        """Compose the footer shell layout."""
        with Vertical(id="shell-output-container"):
        ...
        with Horizontal(id="prompt-container"):
            ...
            yield Input(id="shell-input", placeholder="Type command (Ctrl+` to focus)...")
    ...
    def expand(self) -> None:
        """Expand the shell to semi-modal overlay."""
        self.expanded = True
        self.call_later(self._focus_input)

    def collapse(self) -> None:
        """Collapse the shell back to footer bar."""
        self.expanded = False
    ...
    async def on_key(self, event) -> None:
        """Handle key events for history, completion, and expand/collapse."""
        try:
            input_widget = self.query_one("#shell-input", Input)
        except Exception:
            return
        ...
        # Expand when user starts typing (if not already expanded)
        if not self.expanded and event.key not in ("ctrl+c", "escape", "ctrl+grave_accent"):
            self.expand()
