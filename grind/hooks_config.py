...
The hooks use python3 stdlib (urllib) to POST JSON payloads. This keeps
the hook commands dependency-free — no curl or jq required.
...
def generate_hooks_config(
    ...
    These hooks fire python3 commands that POST event data to the
    observer server. Claude Code passes event data via stdin as JSON.
...
def _build_hook_command(event_type: str, endpoint: str) -> str:
    """Build the python3 command for a hook.
    ...
    Errors are silenced so a dead observer never breaks the Claude session.
    """
    return (
        f'python3 -c "'
